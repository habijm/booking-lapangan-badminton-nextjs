...
interface BookingDetail {
  ...
  court: { id: string; name: string; price_per_hour: number } | null;
}
...
export default function BookingStatusPage() {
  ...
  // Auto-poll kalau masih pending (tiap 5 detik)
  useEffect(() => {
    if (!booking) return;
  ...
  if (!bookingId) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#0A1F12' }}>
      <div className="text-center space-y-3 p-6">
        ...
        <button onClick={() => router.push('/')} className="mt-4 px-6 py-2.5 rounded-xl bg-[#40916C] text-white text-sm font-bold">
          Ke Halaman Utama
        </button>
      </div>
    </div>
  ...
  if (error || !booking) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#0A1F12' }}>
      <div className="text-center space-y-3 p-6">
        ...
        <button onClick={() => router.push('/')} className="mt-2 px-6 py-2.5 rounded-xl bg-[#40916C] text-white text-sm font-bold">
          Ke Halaman Utama
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen py-8 px-4" style={{ background: '#0A1F12' }}>
      <div className="max-w-md mx-auto space-y-4">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.push('/')}
            className="w-9 h-9 rounded-xl border border-[#52B788]/20 flex items-center justify-center text-[#74C69D]/50 hover:text-[#74C69D] hover:border-[#52B788]/40 transition-all text-sm"
          >
            <Icon name="chevronLeft" size={16} />
          </button>
          <div>
            <h1 className="font-bold text-white text-lg font-display leading-tight">Status Booking</h1>
        ...
        <div className="rounded-2xl border border-[#52B788]/15 overflow-hidden" style={{ background: '#0D2B1C' }}>
          ...
          <div className="px-5 py-4 space-y-3">
            {[
              ...
              { icon: 'clock' as IconName, label: 'Jam', value: `${booking.start_time} – ${booking.end_time} WIB` },
              ...
              <div key={label} className="flex justify-between items-start gap-3 text-sm">
                <span className="text-[#74C69D]/50 flex-shrink-0 text-xs inline-flex items-center gap-1.5"><Icon name={icon} size={13} />{label}</span>
        ...
        <div className="rounded-2xl border border-[#52B788]/15 overflow-hidden" style={{ background: '#0D2B1C' }}>
          ...
          <div className="px-5 py-4 space-y-3">
            {[
              ...
              <div key={label} className="flex justify-between items-start gap-3 text-sm">
                <span className="text-[#74C69D]/50 flex-shrink-0 text-xs inline-flex items-center gap-1.5"><Icon name={icon} size={13} />{label}</span>
                <span className="text-white font-medium text-right text-xs font-mono">{value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          {isPending && booking.snap_url && (
            <a
              href={booking.snap_url} target="_blank" rel="noopener noreferrer"
        ...
        {lastCheck && (
          <p className="text-center text-[10px] text-[#74C69D]/25">
            Terakhir diperiksa: {format(lastCheck, 'HH:mm:ss', { locale: id })}
