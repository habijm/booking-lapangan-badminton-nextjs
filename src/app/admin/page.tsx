...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session }, error: sessionError }) => {
      ...
      if (sessionError) {
        setError('Gagal memverifikasi sesi. Silakan refresh halaman.');
        return;
      }
      ...
      if (!cancelled) {
        setCheckingSession(false);
        setError('Gagal memeriksa sesi autentikasi.');
      }
    });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden"
      ...
      {checkingSession ? (
        <div className="min-h-screen flex items-center justify-center gap-3" style={{ background: '#0D1F16' }}>
          <span className="inline-block w-5 h-5 border-2 border-[#52B788]/20 border-t-[#52B788] rounded-full animate-spin"/>
          <span className="text-sm font-medium" style={{ color: 'rgba(116,198,157,0.7)' }}>Memverifikasi sesi...</span>
        </div>
      ) : (
        <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden"
          style={{ background: '#0D1F16' }}>
          ...
          <div className="relative z-10 w-full max-w-md">
            ...
            <div className="text-center mb-8">
              <div className="w-16 h-16 rounded-2xl bg-[#40916C] flex items-center justify-center mx-auto mb-4 shadow-lg shadow-[#40916C]/30">
                <Icon name="court" size={32} className="text-white" />
              </div>
